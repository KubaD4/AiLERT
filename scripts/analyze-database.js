...
async function analyzeDatabase() {
    ...
    try {
        ...
        if (analysis.relationships.cameraStream.camerasWithStream < analysis.summary.cameras) {
            const missing =
                analysis.summary.cameras -
                (analysis.relationships.cameraStream.camerasWithStream || 0);
            analysis.recommendations.push({
                priority: "MEDIUM",
        ...
        if (analysis.detectionService.existingCameras < analysis.detectionService.configuredIds) {
            const missing =
                analysis.detectionService.configuredIds - analysis.detectionService.existingCameras;
            analysis.recommendations.push({
                priority: "HIGH",
