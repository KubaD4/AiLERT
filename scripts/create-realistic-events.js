...
const EVENT_TEMPLATES = {
    incidenti: [
        {
            ...
            description: "Tamponamento tra due veicoli",
            severity: "media",
        },
        {
            title: "Incidente con feriti",
            description: "Scontro con intervento ambulanza",
            severity: "alta",
        },
        { title: "Incidente lieve", description: "Collisione senza feriti", severity: "bassa" },
        {
            title: "Incidente moto-auto",
            description: "Urto tra motociclo e automobile",
            severity: "media",
        },
        { title: "Incidente multiplo", description: "Coinvolti 3 veicoli", severity: "alta" },
        {
            title: "Fuoriuscita autonoma",
            description: "Veicolo finito fuori carreggiata",
            severity: "bassa",
        },
        { title: "Incidente con mezzi pesanti", description: "Camion coinvolto", severity: "alta" },
        {
            title: "Scontro frontale",
            description: "Impatto frontale tra due auto",
            severity: "alta",
        },
    ...
    ingorghi: [
        {
            title: "Ingorgo traffico intenso",
            description: "Code dovute all'ora di punta",
            severity: "media",
        },
        {
            title: "Rallentamenti lavori",
            description: "Cantiere stradale causa rallentamenti",
            severity: "bassa",
        },
        {
            title: "Blocco stradale",
            description: "Strada temporaneamente chiusa",
            severity: "alta",
        },
        {
            title: "Ingorgo centro città",
            description: "Traffico intenso zona centrale",
            severity: "media",
        },
        {
            title: "Code per evento",
            description: "Rallentamenti per manifestazione",
            severity: "bassa",
        },
        {
            title: "Ingorgo zona commerciale",
            description: "Traffico intenso centro commerciale",
            severity: "media",
        },
        {
            title: "Rallentamenti pioggia",
            description: "Traffico lento per condizioni meteo",
            severity: "bassa",
        },
        {
            title: "Code uscita autostrada",
            description: "Rallentamenti casello autostradale",
            severity: "media",
        },
        {
            title: "Ingorgo zona universitaria",
            description: "Traffico intenso orario lezioni",
            severity: "media",
        },
        {
            title: "Blocco trasporto pubblico",
            description: "Interruzione linea autobus",
            severity: "alta",
        },
    ],
};
    ...
    try {
        ...
        for (let i = 0; i < NUM_INCIDENTS; i++) {
            try {
                ...
                if (savedEvent.location?.coordinates?.lat) {
                    ...
                    console.log(
                        `   Coordinate: ${savedEvent.location.coordinates.lat.toFixed(4)}, ${savedEvent.location.coordinates.lng.toFixed(4)}`
                    );
                } else {
        ...
        for (let i = 0; i < NUM_TRAFFIC; i++) {
            try {
                ...
                console.log(`   Status: ${eventData.status} | Severity: ${eventData.severity}`);
                console.log(
                    `   Coordinate: ${nearbyCoords.lat.toFixed(4)}, ${nearbyCoords.lng.toFixed(4)}`
                );
            } catch (error) {
                    ...
                    _id: {
                        $switch: {
                            branches: [
                                ...
                                {
                                    case: { $gte: ["$eventDate", oneWeekAgo] },
                                    then: "Ultima settimana",
                                },
                                {
                                    case: { $gte: ["$eventDate", oneMonthAgo] },
                                    then: "Ultimo mese",
                                },
                                {
                                    case: { $gte: ["$eventDate", threeMonthsAgo] },
                                    then: "Ultimi 3 mesi",
                                },
                            ],
        ...
        if (
            eventsWithCoords === finalEvents &&
            recentResults.errori + historicalResults.errori === 0
        ) {
        ...
        if (sampleIncident) {
            ...
            console.log(
                `  Status: ${sampleIncident.status} | Severity: ${sampleIncident.severity}`
            );
        }
        ...
        if (sampleHistorical) {
            ...
            console.log(
                `  Status: ${sampleHistorical.status} | Severity: ${sampleHistorical.severity}`
            );
        }
