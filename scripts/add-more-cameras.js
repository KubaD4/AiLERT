...
async function geocodeStreet(streetQuery) {
    try {
        ...
        if (data && data.length > 0) {
            ...
            const inTrento =
                coordinates.lat >= 46.02 &&
                coordinates.lat <= 46.1 &&
                coordinates.lng >= 11.05 &&
                coordinates.lng <= 11.2;
...
async function getNextCameraNumber() {
    ...
    const cameras = await Camera.find({ name: /^CAM-\d+$/ })
        .sort({ name: -1 })
        .limit(1);
...
async function addMoreCameras() {
    try {
        ...
        for (let i = 0; i < NEW_TRENTO_STREETS.length; i++) {
            ...
            try {
                ...
                installationDate.setMonth(
                    installationDate.getMonth() - Math.floor(Math.random() * 36)
                );
                ...
                const cameraData = {
                    ...
                    modelInfo: {
                        brand: ["Hikvision", "Dahua", "Axis", "Bosch"][
                ...
                console.log(
                    `Camera creata: ${coordinates.lat.toFixed(6)}, ${coordinates.lng.toFixed(6)}`
                );
        ...
        if (results.camerasAdded > 0) {
            ...
            const newCameras = await Camera.find({
                name: { $regex: `^CAM-0(${String(startNumber).padStart(2, "0")}|[1-9][0-9])$` },
            }).sort({ name: 1 });
...
async function analyzeDistribution() {
    try {
        ...
        cameras.slice(-5).forEach(camera => {
            ...
            console.log(
                `  ${camera.name}: ${camera.location.address} (${coords.lat.toFixed(4)}, ${coords.lng.toFixed(4)})`
            );
        });
    } catch (error) {
