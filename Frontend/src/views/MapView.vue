...
<template>
   <div class="map-page"
    > <div class="max-w-7xl mx-auto px-4 py-8"
      > <h1 class="text-3xl font-bold text-gray-900 mb-2 text-center">Mappa Eventi</h1> <p
        class="text-center text-gray-600 mb-6"
        > Visualizza gli eventi recenti a Trento <span
          v-if="events.length === 0 && !loading"
          class="italic"
          > • Nessun evento nelle ultime 2 ore </span
        > </p
      ...
      > <div
        ...
        > {{ error }} <button @click="fetchEvents" class="ml-2 text-blue-600 hover:underline"
          > Riprova </button
        > </div
      > <EventsMap :events="events" :isLoading="loading" @refresh="fetchEvents" /> <div
        v-if="events.length > 0"
        class="mt-12"
        > <h2 class="text-2xl font-bold text-gray-900 mb-4">Eventi recenti</h2> <div
          class="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3"
          > <EventCard v-for="event in events" :key="event._id" :event="event" /> </div
        > </div
      > </div
    > </div
  >
