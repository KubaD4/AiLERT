...
<template>
  <div>
    ...
    <!-- Loading state -->
    <div v-if="isLoading" class="bg-white rounded-2xl shadow-md p-8 flex justify-center items-center">
      <div class="flex flex-col items-center">
        <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mb-4"></div>
        <p class="text-gray-500">Caricamento dettagli utente...</p>
      </div>
    </div>
    ...
    <div v-else class="bg-white rounded-2xl shadow-md overflow-hidden">
      ...
      <div class="p-6 border-b border-gray-200">
        <h2 class="text-2xl font-bold text-gray-800">Dettagli Utente</h2>
      </div>
      ...
      <div class="p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p class="text-base text-gray-900">{{ userData.email || 'N/A' }}</p>
          </div>
          
          <div>
            <h3 class="text-sm font-medium text-gray-500 mb-1">Nome</h3>
            <p class="text-base text-gray-900">{{ userData.name || 'Non specificato' }}</p>
          </div>
          
          <div>
            <h3 class="text-sm font-medium text-gray-500 mb-1">Ruolo</h3>
            <p class="text-base text-gray-900">{{ roleNames[userData.role] || userData.role }}</p>
          </div>
        </div>
    ...
    <div v-if="confirmDeleteModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-xl shadow-xl p-6 max-w-md w-full">
        <h3 class="text-xl font-bold text-gray-900 mb-4">Conferma eliminazione</h3>
