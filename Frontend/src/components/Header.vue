...
const userName = ref('');
const userEmail = ref('');
// Nuova variabile per controllare lo stato del menu mobile
...
onMounted(() => {
  ...
  // Chiudi menu mobile su cambio di route
  router.afterEach(() => {
    isMobileMenuOpen.value = false;
    document.body.classList.remove('overflow-hidden');
  });
...
<template>
  <header class="sticky top-0 z-[9999] bg-white border-b border-gray-100">
    <div class="max-w-7xl mx-auto px-4">
      <nav class="flex items-center justify-between h-16">
        ...
        <div class="md:hidden flex items-center space-x-4 z-20 relative">
          <!-- Login button su mobile quando non loggato -->
          <router-link 
            v-if="!isLoggedIn" 
            to="/login" 
          ...
          <!-- Avatar utente su mobile quando loggato -->
          <div v-if="isLoggedIn" class="w-7 h-7 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 flex items-center justify-center text-white text-sm font-semibold shadow-sm">
            {{ userName.charAt(0).toUpperCase() }}
          </div>
    ...
    <!-- Mobile Menu Overlay - Si apre a schermo intero -->
    <transition
    ...
    >
      <div 
        ...
        class="fixed inset-0 bg-white z-[9999] md:hidden"
      >
        <div class="h-full overflow-y-auto pt-20 pb-6 px-6">
          <!-- Navigation Links Mobile -->
          <div class="flex flex-col space-y-4 text-lg mb-8">
            <button @click="navigateAndCloseMenu('/')" 
              class="flex items-center py-3 px-4 rounded-lg hover:bg-blue-50 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 text-blue-600" viewBox="0 0 20 20" fill="currentColor">
                <path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z" />
              </svg>
              Home
            </button>

            <button @click="navigateAndCloseMenu('/map')" 
              class="flex items-center py-3 px-4 rounded-lg hover:bg-blue-50 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 text-blue-600" viewBox="0 0 20 20" fill="currentColor">
            ...
            <!-- Link Dashboard per utenti non admin -->
            <button 
              v-if="isLoggedIn && !userIsAdmin" 
            ...
            <!-- Link Telecamere per utenti non admin -->
            <button 
              v-if="isLoggedIn && !userIsAdmin" 
              @click="navigateAndCloseMenu('/cameras')" 
              class="flex items-center py-3 px-4 rounded-lg hover:bg-blue-50 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 text-blue-600" viewBox="0 0 20 20" fill="currentColor">
                <path d="M2 6a2 2 0 012-2h6a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V6zM14.553 7.106A1 1 0 0014 8v4a1 1 0 00.553.894l2 1A1 1 0 0018 13V7a1 1 0 00-.447-.894l-2 1z" />
              </svg>
              Telecamere
            </button>

            <!-- Link Admin per admin -->
            <button 
              v-if="isLoggedIn && userIsAdmin" 
          ...
          <!-- Login button in menu for non-logged users -->
