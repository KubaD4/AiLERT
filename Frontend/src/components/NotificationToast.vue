...
onMounted(() => {
  ...
  setTimeout(() => {
    visible.value = true;
    // Start progress bar animation after toast appears
    setTimeout(() => {
      ...
      <div class="p-4">
        <div class="flex items-start">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div v-if="notification.location" class="mt-2 text-xs text-gray-500">
              <div v-if="notification.location.address">
                📍 {{ notification.location.address }}
              </div>
              <div>
                🕒 {{ new Date(notification.eventDate).toLocaleString('it-IT') }}
              </div>
            </div>
          </div>
        </div>
