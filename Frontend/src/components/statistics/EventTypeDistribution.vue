...
const fractions = computed(() =>
  total.value === 0 ? chartData.value.map(() => 0) : chartData.value.map(i => i.value / total.value)
);
...
<template>
   <div class="bg-white rounded-2xl shadow-md p-6"
    > <h2 class="text-xl font-bold text-gray-900 mb-6">Distribuzione per Tipo di Evento</h2> <div
    ...
    > <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4"
      > <div class="flex flex-col justify-center space-y-4"
        > <div class="p-4 bg-red-50 rounded-xl"
          > <div class="text-sm font-medium text-red-800">Incidenti</div> <div
            class="flex items-end justify-between mt-1"
            > <div class="text-2xl font-bold text-red-600">{{ chartData[0].value }}</div
            > <div class="text-sm font-medium text-red-600"
              >{{ Math.round(fractions[0] * 100) }}%</div
            > </div
          > </div
        > <div class="p-4 bg-orange-50 rounded-xl"
          > <div class="text-sm font-medium text-orange-800">Ingorghi</div> <div
            class="flex items-end justify-between mt-1"
            > <div class="text-2xl font-bold text-orange-600">{{ chartData[1].value }}</div
            > <div class="text-sm font-medium text-orange-600"
              >{{ Math.round(fractions[1] * 100) }}%</div
            > </div
          > </div
        > </div
      > <div class="md:col-span-2 flex justify-center items-center"
        > <div class="relative w-40 h-40 flex justify-center items-center"
          > <div class="absolute z-10 flex flex-col items-center justify-center"
            > <div class="text-sm text-gray-600">Totale</div> <div
              class="text-2xl font-bold text-gray-800"
              >{{ total }}</div
            > <div class="text-xs text-gray-500">eventi</div> </div
          > <svg viewBox="0 0 100 100" class="absolute inset-0 w-full h-full">
            ...
            <template v-for="(slice, i) in chartData" :key="slice.name">
              ...
              <path v-if="fractions[i] > 0" :d="getPieSegment(i, fractions)" :fill="slice.color" />
      ...
      > <div class="md:col-span-3 flex justify-center space-x-8 mt-4"
        > <div class="flex items-center"
          > <div class="w-4 h-4 bg-red-500 rounded-full mr-2"></div> <span>Incidenti</span> </div
        > <div class="flex items-center"
          > <div class="w-4 h-4 bg-orange-500 rounded-full mr-2"></div> <span>Ingorghi</span> </div
        > </div
      > </div
    > </div
  >
