    ...
    <div v-else class="space-y-6">
      <div v-for="(item, index) in chartData" :key="index" class="mb-4">
        ...
        <div class="h-8 bg-gray-100 rounded-lg overflow-hidden flex">
          ...
          >
            <span v-if="item.incidenti > 1" class="text-white text-xs font-semibold">{{ item.incidenti }}</span>
          </div>
          ...
          >
            <span v-if="item.ingorghi > 1" class="text-white text-xs font-semibold">{{ item.ingorghi }}</span>
          </div>
        </div>
      </div>
